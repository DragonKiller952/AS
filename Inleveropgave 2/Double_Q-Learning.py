...
class Maze:
    def __init__(self):
        ...
        for y in range(4):
            for x in range(4):
                if [x, y] == [3, 0]:
                    self.grid[x, y] = State(x=x, y=y, reward=40, Q={'↑': 0, '→': 0, '↓': 0, '←': 0},
                                            Q1={'↑': 0, '→': 0, '↓': 0, '←': 0},
                                            Q2={'↑': 0, '→': 0, '↓': 0, '←': 0}, finished=True)
                elif [x, y] == [2, 1] or [x, y] == [3, 1]:
                    self.grid[x, y] = State(x=x, y=y, reward=-10, Q={'↑': 0, '→': 0, '↓': 0, '←': 0},
                                            Q1={'↑': 0, '→': 0, '↓': 0, '←': 0},
                                            Q2={'↑': 0, '→': 0, '↓': 0, '←': 0}, finished=False)
                elif [x, y] == [0, 3]:
                    self.grid[x, y] = State(x=x, y=y, reward=10, Q={'↑': 0, '→': 0, '↓': 0, '←': 0},
                                            Q1={'↑': 0, '→': 0, '↓': 0, '←': 0},
                                            Q2={'↑': 0, '→': 0, '↓': 0, '←': 0}, finished=True)
                elif [x, y] == [1, 3]:
                    self.grid[x, y] = State(x=x, y=y, reward=-2, Q={'↑': 0, '→': 0, '↓': 0, '←': 0},
                                            Q1={'↑': 0, '→': 0, '↓': 0, '←': 0},
                                            Q2={'↑': 0, '→': 0, '↓': 0, '←': 0}, finished=False)
                else:
                    self.grid[x, y] = State(x=x, y=y, reward=-1, Q={'↑': 0, '→': 0, '↓': 0, '←': 0},
                                            Q1={'↑': 0, '→': 0, '↓': 0, '←': 0},
                                            Q2={'↑': 0, '→': 0, '↓': 0, '←': 0}, finished=False)
    ...
    def Q_learning(self):
        ...
        for episode_num in tqdm(range(100000)):
            ...
            while not self.location.finished:
                ...
                if random_value < 0.5:
                    max_value = max(newlocation.Q1.values())
                    max_key = [key for key, value in newlocation.Q1.items() if value == max_value][-1]
                    newaction = max_key
                    location.Q1[action] += learning_rate * (reward +
                                                            (self.discount * newlocation.Q2[newaction]) -
                                                            location.Q1[action])
                else:
                    max_value = max(newlocation.Q2.values())
                    max_key = [key for key, value in newlocation.Q2.items() if value == max_value][-1]
                    newaction = max_key
                    location.Q2[action] += learning_rate * (reward +
                                                            (self.discount * newlocation.Q1[newaction]) -
                                                            location.Q2[action])
                location.Q[action] = location.Q1[action] + location.Q2[action]
